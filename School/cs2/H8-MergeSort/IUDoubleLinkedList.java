...
//		}
	...
	public T removeFirst() {
		...
		// this thing
		// correctly, it'll still behave, and it'll still throw the appropriate
		// exceptions at the
		// appropriate times. Let's see if I'm right
		DLLIterator scamp = new DLLIterator();
...
//		return retVal;
	...
	public T remove(T element) {
		...
		// CAN'T, because
		// chopBlock only stays null if we don't find the thing we're looking for. If we
		// don't find
		// the thing we're looking for, the NSE Exception above triggers. Otherwise,
		// we've found it
...
//		return choppingBlock;
	...
	public void set(int index, T element) {
		...
		// their old
		// validity checks in place or tests start failing. I'm not sure why that sort
		// of check isn't
		// integrated into the iterator. Maybe it COULD be and I just didn't, and
		// leaving those
		// checks in place is just easier. Hrm. That's a slight disappointment. Maybe
		// I'll screw
		// with it more, later
		DLLIterator scamp = new DLLIterator(index);
		scamp.next();
...
//		modCount++;		
	...
	public T get(int index) {
		...
		// doing
		// in this new-and-improved, listIterator-centric version. But... maybe that's a
		// plus to people who ain't me
		DLLIterator scamp = new DLLIterator(index);
...
//		}
	...
	public int indexOf(T element) {
		...
		// return -1 if
		// the element is never found. Therefore, what-EVER happens, we spit out
		// indyQuery at the
		// end; either it's the value of the target, or it's the -1 we set it to to
		// begin with
		return indyQuery;
...
//		stringKing.append("]");
	...
	private class DLLIterator implements ListIterator<T> {
		...
		// which will enable
		// the act of removing to begin with, we just need this second boolean to act as
		// the fork for which
		// direction we're removing
		...
		public void remove() {
			...
			// has been called. Now we need to know which
			...
			// All other cases
				...
				// the next and prev
				// references of the nodes surrounding markedOne; we can still use it to get
				// these values
				if (wasNext)
		...
		// if so, regress
		// the iterator and return that value
		public T previous() {
			if (!hasPrevious())
				throw new NoSuchElementException();
